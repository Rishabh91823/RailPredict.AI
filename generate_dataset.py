...
def generate_1cr_dataset():
    ...
    np.random.seed(42)
    n_samples = 10_000_000  # Exactly 1 Crore rows
    ...
    train_type = np.random.randint(0, 3, size=n_samples, dtype=np.uint8) # 0: Express, 1: Rajdhani, 2: Vande Bharat
    travel_class = np.random.randint(0, 4, size=n_samples, dtype=np.uint8) # 0: SL, 1: 3A, 2: 2A, 3: 1A
    quota = np.random.randint(0, 2, size=n_samples, dtype=np.uint8) # 0: GN, 1: TQ
    is_festival = np.random.choice([0, 1], size=n_samples, p=[0.8, 0.2]).astype(np.uint8)

    # 2. Mathematical Probability Engine
    ...
    # Confirmed target (1 if probability > 50%, else 0)
    ...
    # Save to CSV in optimized format
